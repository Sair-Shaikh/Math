        ...
        with open(file, "r") as f:
            for line in f:
                ...
                if key_match:
                    ...
                    print(key, in_cpp_block, cpp_lines, line)
...
def process_chunk(chunk):
    ...
    curr_results = {}
    try: 
        compile_cmd = ["g++","-std=c++17", f"-include{header_path}", str(CPP_FILE),"-o", str(executable),"-O2", f"-DBATCH_SIZE={min(BATCH_SIZE,len(results))}", "-DEXT_COEFFS=1", f"-DN={N}"]
...
# CPP_FILE = Path("CppLib/count_cubic_fast.cpp")
# CSV_FILE = BASE_PATH / f"point_counts_cubic_bigq_{N}_fast.csv"
# DATA_FILES = [f"Dataset/cpp_coeffs/cubic_bigq_{N}_fast.txt" ]

# For surfaces with a line
CPP_FILE = Path("CppLib/count_quad_fast_test.cpp")
CSV_FILE = BASE_PATH / f"point_counts_quad_bigq_{N}.csv"
DATA_FILES = [f"Dataset/cpp_coeffs/quad_coeffs_{N}_fast_test.txt"]
