...
\bbni 
\begin{enumerate}
    \item Assume $F: \CC \to \DD$ is an equivalence of categories and there exists an isomorphism $f \in \hom_\DD(F(X), F(Y))$. Since $F$ is an equivalence of categories, there exists functor $F': \DD \to \CC$ such that $F' \circ F$ is naturally isomorphic to $\id_\CC$. Since $F'$ is a functor, $F'(f) \in \hom_\CC(F'F(X), F'F(Y))$ is an isomorphism. Moreover, we have isomorphisms $\eta_X \in \hom_\CC(F'F(X), X)$ and $\eta_Y \in \hom_\CC(F'F(Y), Y)$. Thus, we have an isomorphism:
