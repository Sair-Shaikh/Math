import "magma_scripts/Utils.m" : ConvertToPolys, CppHeaderTextQuadBigQ;
...
fname := Sprintf("Dataset/cpp_coeffs/quad_coeffs_bigq_%o.txt", N); // Writing those that need counting up to 12
F := Open(fname, "w");
delete F; 
...
for key in Keys(orbits) do
    ...
    str := "Key: " cat key cat "\n";
    str cat:= CppHeaderTextQuadBigQ(A, B, C, A2, B2, C2);
    str cat:= "\n";
