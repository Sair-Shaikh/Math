...
#ifndef EXT_COEFFS
#include "batched_coeffs_quad_fast.h"
#endif
...
// function prototypes
int* contribution_of_fibre_over_P2_point(unsigned, unsigned, unsigned, unsigned, unsigned __int128*, unsigned __int128*, unsigned __int128*);
...
int main(int argc, char **argv) {
    ...
    unsigned __int128 *As = new unsigned __int128[BATCH_SIZE], *Bs = new unsigned __int128[BATCH_SIZE], *Cs = new unsigned __int128[BATCH_SIZE];
    BATCHED_ABC2;
...
int* contribution_of_fibre_over_P2_point(unsigned q, unsigned y_0, unsigned y_1, unsigned y_2, unsigned __int128 *As, unsigned __int128 *Bs, unsigned __int128 *Cs) {
    ...
    for (int i = 0; i < BATCH_SIZE; i++) {
        unsigned __int128 Aint = As[i], Bint = Bs[i], Cint = Cs[i];
        unsigned A = to_coeff(Aint, mons), B = to_coeff(Bint, mons), C = to_coeff(Cint, mons);
        // printf("A=%d\n", A);
        // printf("B=%d\n", B);
        // printf("C=%d\n", C);
